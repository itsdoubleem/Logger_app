...
**폰에서 확인**(EN, 실제 기기): 노란 `DAY` 알약 안에 검은 해, 그 오른쪽
`09:00 start` 앞에 회색 해. 근무조를 야간으로 두고 다시 보니 검은 `NIGHT` 알약 안에
흰 달, `21:00 start` 앞에 회색 달입니다. 근로자의 저장소는 **백업 → 시험 → 복원**
했고 파싱한 JSON이 백업과 완전히 같습니다(기록 32일, `shifts: both` 그대로).
