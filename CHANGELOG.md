...
이번에는 grep으로 셌습니다: W를 받는 함수(`dayPay`, `legalGap`, `avgDaily`, `avgIsEst`,
`partialShort`, `partialWorkedPay`)를 W 없이 부르는 곳은 이제 둘입니다 — 설정 탭의 지금 회사
안내(오늘이 맞습니다)와 `totals()`의 `t.pay`(어디서도 읽지 않는 합입니다. 돈은 `payCalc(t, W)`가
시간에서 W로 다시 셉니다).
