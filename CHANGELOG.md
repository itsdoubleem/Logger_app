...
열린 기간 고르기는 13px만 짧아졌습니다. 나머지는 44px 달 칸 두 줄과 연도 줄이고, 그것을
...
#### 리뷰가 잡은 것 (10) — 앞날의 정산이 '기록이 부족'을 빠뜨렸습니다

(1)이 붙인 추정 줄은 앞날이면 언제나 "미리 본 값"이라고만 했습니다. 09.01부터만 기록이 있는
사람(창의 48%)의 12.31 정산은 통상임금 바닥에 걸린 값인데, 바로 옆 퇴직금 카드는 같은 물음에
"기록이 부족"이라고 답했습니다. 큰 숫자를 내는 쪽이 그것이 낮을 수 있다는 것을 감췄습니다.
이제 둘 다이면 "예 — 기록이 부족한 지난 3개월로 미리 본 값".

#### 리뷰가 잡은 것 (11) — 마지막 근무일 당일의 정산이 전날보다 적었습니다

오늘이 마지막 근무일이면 창이 오늘에서 끝났는데, 오늘의 근무는 퇴근을 찍기 전까지 기록이
없습니다. 그 평일이 무급처럼 셈해져서 정산이 전날 ₩9,513,752에서 당일 ₩9,486,948로
내려갔습니다 — 근로자가 이 카드를 가장 많이 열 바로 그 날입니다. 이제 당일도 앞날처럼
평균임금을 어제까지의 3개월로 미리 봅니다. 퇴직금 카드도 같은 창을 써서, 당일에도 두 카드가
같은 금액을 말합니다. 근속은 여전히 당일까지 셉니다.

#### 리뷰가 잡은 것 (12) — 새 회사 입사일을 적는 순간 굳힘이 풀렸습니다

사업장을 옮긴 E-9 근로자가 가장 흔히 하는 일은 새 입사일과 새 기본금을 적는 것입니다.
입사일이 마지막 근무일을 넘는 순간 `lastDay()`가 null이 되고, (3)의 굳힘이 풀려 옛 회사의
아직 열린 마지막 기간이 새 회사 기본금으로 다시 찍혔습니다. 이제 굳힘은 근로자가 적은 날
그대로(`lastDayRaw`)를 봅니다. 그 날이 든 한 기간만 붙들므로, 칸에 남은 옛 날짜가 지금
회사의 기간을 건드리지 않습니다. 그래서 '입사일보다 앞선 날'은 더 이상 오류가 아닙니다 —
빨간 테두리와 "비우십시오"를 거두고, 옛 회사의 마지막 근무일로 읽는다고 말합니다. 비우면
붙든 것이 풀리기 때문입니다.

한계: 옛 회사의 마지막 날과 새 회사의 첫날이 **같은 급여기간**에 들면, 그 기간은 한 벌의
임금밖에 가질 수 없어 옛 회사의 값으로 셉니다. 그 기간의 급여 탭에 적힌 새 회사 날들은 옛
기본금입니다. 사업장 변경에는 대개 몇 주가 걸려 드문 경우입니다.

#### 리뷰가 잡은 것 (13) — 적는 순서에 따라 새 기본금이 굳었고, 화면은 말이 없었습니다

(3)의 굳힘은 그 순간의 도장을 붙듭니다. 새 회사 기본금을 **먼저** 적고 마지막 근무일을
**나중에** 적으면 도장은 이미 새 값이라 그것이 굳습니다(정산 ₩10,151,040 — (3)이 막으려던 바로
그 금액). 앱은 도장의 이전 값을 기억하지 않아 스스로 되살릴 수 없습니다. 이제 마지막 근무일 칸
밑에 그 기간에 굳은 기본금을 보여 주고(`heldNote`), 기간이 아직 열려 있으면 고치는 길 —
비우고, 기본금을 고치고, 다시 적기 — 을 말합니다. 시험이 그 길을 끝까지 걸어 옛 기본금이 굳는
것을 봅니다. 닫힌 기간은 어느 기간이든 다시 찍히지 않으므로 값만 보여 줍니다.

남은 것: 급여 탭 쪽에는 이 말이 없습니다. 그 기간의 급여 탭에서 기본금을 바꿔도 금액이
움직이지 않는 이유는 내 권리의 이 칸 밑에만 적혀 있습니다.

#### 리뷰가 잡은 것 (14) — v1.1.0이 찍은 도장에는 두 값이 없었습니다 (v1.2.0 뒤)

(9)는 상여와 직접 적은 평균임금을 도장에 실었지만, 그 전 판이 찍은 도장에는 둘 다 없고,
없으면 지금 설정으로 떨어졌습니다. 1.1.0을 쓰다 그만둔 사람 — 정산 카드를 처음 여는 바로 그
사람 — 에게 덫이 열려 있었습니다: 새 회사를 위해 1일 평균임금 ₩150,000을 적으면 옛 퇴직금이
₩8,271,833 → ₩15,028,767. 이제 앱을 열 때와 백업을 불러올 때 한 번(`fillStampFields`), 두 값이
없는 도장에 그 순간의 설정값을 채웁니다. 그 순간의 설정이 가진 가장 나은 증거이고, 그 뒤로는
옛 기간이 움직이지 않습니다. 다만 **금액은 나빠지지 않지만 되돌릴 길은 줄었습니다** — (18) 참고. 이미 값이 있는 도장은
건드리지 않습니다.

#### 리뷰가 잡은 것 (15) — '추정입니까'와 그 옆의 금액이 서로 다른 곳을 봤습니다

(9) 뒤로 1일 평균임금은 도장의 `avgManual`을 읽는데, "낮게 추정된 값" 단서(`avgIsEst`)는
여전히 지금 설정을 읽었습니다 — (7)과 같은 종류입니다. 모든 근로자의 근무내역서에 닿습니다:
도장 0 / 설정 150,000이면 통상임금 바닥 ₩82,560이 단서 없이 확정처럼 찍혔고, 도장 90,000 /
설정 0이면 근로자가 명세서에서 옮긴 ₩90,000을 "앱이 낮게 추정한 값"이라고 불렀습니다. 이제
`avgIsEst(on, W)`가 `avgDaily`와 같은 규칙으로 읽고, 근무내역서·정산·퇴직금 카드의 세 자리가
W를 넘깁니다.

#### 리뷰가 잡은 것 (16) — 근속 몇 년 몇 개월이 아직 365일로 셌습니다

(8)이 퇴직금과 연차를 달력으로 옮기면서 근속 문장(`tenureYM`)은 365일 해에 남았고, 카드가 바로
그 경우들에서 스스로와 어긋났습니다: 2023.03.01 입사자가 2024.02.28에 "근속 1년 0개월" 위에
"1년 미만 … 1일 남았습니다", 2020.03.01 입사자가 "4년 0개월 … 다음 입사기념일까지 2일". 날수 %
365가 360을 넘는 날에는 "0년 12개월"도 나왔습니다. 이제 근속은 그 날까지(그 날 포함) 채운
달력의 해와 달 — 기념일 전날에 1년, 퇴직금의 1년과 같은 셈입니다. 날수는 "(N일)"과 ×일수/365에만
씁니다. 그래서 위의 시험 하나가 고쳐졌습니다: 2024.03.04 → 2026.10.30은 2년 **7**개월입니다.

기념일 전날 그만둔 사람은 퇴직금의 1년은 채웠지만 15일은 없습니다. 연차 대장이 그 이유를 한 줄로
말합니다 — "15일은 1년의 근로를 마친 다음 날 재직 중이어야 생깁니다 (대법원 2021다227100)". 근로자가
회사에 가져갈 바로 그 물음입니다.

#### 리뷰가 잡은 것 (17) — 근무기록의 하루 화면이 제46조 차액을 문서와 다르게 셌습니다

(15)가 고친 종류의 네 번째 자리입니다. 하루 화면의 제46조 줄 가운데 '최저'만 W를 받고, '시간을
깎였다면'의 금액과 차액, '1일 평균임금'은 지금 설정을 읽었습니다. 닫힌 8월(도장의 평균임금 90,000,
설정은 비움)에서 화면은 최저 ₩63,000 − ₩41,280을 "+₩16,512"라고 했고, 같은 날의 근무내역서는
₩21,720을 찍었습니다 — 화면의 셈이 스스로 맞지 않았고, 증거 문서와 ₩5,208 달랐습니다. 이제 네 줄
모두 같은 W를 씁니다. 그 때 'W 없이 부르는 곳은 설정 탭의 지금 회사 안내 한 자리뿐'이라고 적었는데 틀렸습니다 — (19) 참고.

#### 리뷰가 잡은 것 (18) — 붙들린 평균임금과 상여가 화면 어디에도 없었습니다

(14)는 업데이트하는 순간의 설정을 옛 도장에 옮겨 적습니다. 그만둔 사람이 **그 전에** 이미 새 회사의
1일 평균임금이나 상여를 적어 두었다면 그 값이 옛 회사의 닫힌 기간에 붙들립니다(퇴직금
₩15,028,767). v1.2.0에서는 설정을 비우면 ₩8,271,833으로 돌아왔지만, 이제는 붙들려 있고 — 마지막
근무일 칸 밑의 말은 기본금만 말해서 근로자는 그 까닭을 볼 수 없었습니다. 이제 붙들린 1일
평균임금과 상여도 그 자리에 적힙니다(0이면 말하지 않습니다).

**남은 일:** 닫힌 기간에 붙들린 값을 고치는 길은 없습니다. 닫힌 기간은 설계상 다시 찍히지 않고,
v1.2.1을 연 휴대폰에서는 이미 채워졌습니다. 고치려면 지나간 기간의 임금을 손으로 고치는 새 자리가
필요하고, 그것은 화면을 하나 더하는 설계의 문제라 근로자(이 앱의 주인)에게 묻습니다.

#### 리뷰가 잡은 것 (19) — 하루 화면과 급여 탭의 제56조② 차액이 문서와 달랐습니다

(17)에서 "W 없이 부르는 곳은 한 자리뿐"이라고 적었지만, 같은 하루 화면 두 줄 아래와 급여 탭에
네 자리가 더 있었습니다: 하루 화면의 '일일 변동수당'(`dayPay`)과 제56조② 차액(`legalGap`) 두 줄,
급여 탭 제56조② 카드의 금액과 표시 여부. 닫힌 8월, 휴일 08.16에 8시간을 3시간 넘겨 일한 뒤 기본금을
2,156,880 → 2,800,000으로 올리면, 화면과 카드는 차액 +₩20,094, 같은 기간의 근무내역서는 ₩15,480.
holOverPaid를 켜면 하루 화면의 줄은 ₩185,760으로 더해지는데 합계는 ₩190,374였습니다. 이제 모두 그
기간의 W를 쓰고, 카드 밑의 날짜 목록도 같은 W로 거릅니다.

이번에는 grep으로 셌습니다. 그리고 센 김에 하나를 더 찾았습니다: 휴업수당 합계(`shutdownTally(P)`)가
기간을 받으면서 그 기간의 W를 쓰지 않았습니다(닫힌 8월, 기본금을 올린 뒤 ₩75,023 — 그 기간의 값은
₩57,792). 이제 그것도 W로 셉니다. 그 때 "남은 곳은 셋, 셋 다 맞다"고 적었는데 **그것도 틀렸습니다** —
grep이 여섯 함수 이름만 셌고, 남은 곳의 판단도 한 곳(마지막 근무)은 보통 날로만 시험했습니다. 기계로 다시
센 목록은 (21)에 있습니다.

다음 사람에게: "이제 한 자리뿐"이라고 적기 전에 **세십시오.** 셈하지 않은 '뿐'은 다음 리뷰가 찾아냅니다.

#### 리뷰가 잡은 것 (20) — 고치는 길이 기본금만 말했습니다

(18) 뒤로 마지막 근무일 칸 밑의 말은 붙들린 1일 평균임금과 상여도 적는데, 그 뒤의 '고치는 길'은
여전히 "옛 회사의 기본금이 아니라면 … 기본금을 고친 뒤"였습니다. 붙들린 평균임금이 새 회사의 것인
사람은 이미 맞는 기본금을 고치라는 말만 들었습니다 — 기간이 열려 있는 동안이 고칠 수 있는 유일한
때인데도. 이제 "위의 값 가운데 옛 회사의 것이 아닌 것이 있다면 … 그 값을 옛 회사의 것으로 고친 뒤,
이 날을 다시 적으십시오. 그 다음 새 회사의 값을 되돌려 적으면 됩니다." 마지막 한 걸음(되돌려 적기)은
전에 빠져 있었습니다. 시험이 평균임금으로 그 길을 끝까지 걸어 봅니다.

#### 리뷰가 잡은 것 (21) — 날짜를 알면서 오늘로 센 다섯 자리, 그리고 기계로 센 목록

일곱 번째 리뷰가 W 없이 부르는 곳을 다시 세어 (19)의 "셋"이 틀렸음을 보였습니다. 이번에는 사람이 아니라
스크립트가 셌습니다: 마지막 인자가 `W`인 메서드 26개의 모든 호출을 괄호 깊이까지 읽어, 그 인자를 넘기지
않는 곳을 찾았습니다(20곳). 그 가운데 **지난 날짜를 알면서 오늘로 센 다섯**을 고쳤습니다. 모두 새 도우미
`wageOn(d)`(그 날짜가 든 급여기간의 W)를 씁니다:

| 자리 | 전 (닫힌 8월, 기본금을 올린 뒤) | 후 |
|---|---|---|
| 사유 창의 제46조 안내 (`rsnEffect`) | 오늘의 창·오늘의 시급 ₩75,023 | 그 기록의 날짜로 ₩57,792 |
| 휴업을 고를 때의 안내 | ₩75,023 | 고른 날짜로 ₩57,792 |
| 결근을 고를 때의 안내 | 오늘의 시급 ₩107,176 | 고른 날짜의 기간 |
| 근무기록 목록의 결근 줄 | 오늘의 시급 | 그 기간 — 문서의 결근 공제와 같게 |
| 출퇴근 화면의 '마지막 근무' | ₩190,374 (휴일 8시간 초과가 든 날) | ₩185,760 — 문서와 같게 |

출퇴근 화면의 '예상 실수령'도 지금 기간의 W(`wageOn()`)로 바꿨습니다 — 그만둔 기간이 아직 열려 있을
때만 다릅니다. (19)의 시험 주석이 "마지막 근무는 이미 맞았다"고 스스로 모순되던 것도 고쳤습니다(보통
날에서만 맞았습니다).

**남은 W 없는 호출은 14곳이고, 모두 '오늘'이 맞는 자리입니다** — 스크립트로 다시 센 결과: 4대보험 설정의
자동/직접 전환(2), 설정 탭의 4대보험·보수월액·세금 표시(4)와 지금 회사 휴업수당 안내(4), 세금 구간 경고(1),
지금 진행 중인 근무의 '오늘 번 돈'·'지금까지'(2), 그리고 `totals()`의 `t.pay`(1, 아무도 읽지 않는 합).

다음 사람에게: "몇 곳뿐"은 **스크립트로** 세십시오. 사람이 고른 함수 이름 목록으로 grep하면, 목록에 없는
함수가 빠지고 — 이번에 `shutdownPay`와 `absentDeduct`가 그랬습니다.

#### 리뷰가 잡은 것 (22) — 급여 합계와 근무내역서의 휴업 공제가 오늘의 3개월로 셌습니다

제46조의 평균임금은 사유가 생긴 날 — 휴업한 날 — 이전 3개월입니다(제2조①6). 내 권리 합계, 하루
화면, (21)이 고친 고를 때의 안내는 그 날의 창을 썼는데, 급여 계산(`payCalc`)의 휴업 공제와 그것을 찍는
근무내역서, 근무기록의 휴업 줄은 `shutdownPay(null, W)` — 문서를 뽑는 **오늘**의 창이었습니다. 통상임금
바닥에 걸린 사람에게는 두 값이 같아서 지금까지의 시험이 모두 놓쳤습니다. 휴업 전 석 달 매일 잔업한
사람(평균임금이 바닥 위)에게서 문서는 휴업 공제 ₩20,202, 그 날의 창으로는 ₩5,125. 이제 `totals()`가 휴업한
날짜를 모으고, 날마다 그 날의 창으로 셉니다. 닫힌 기간의 문서가 그 뒤의 기록에 흔들리지 않는 것도 시험이
봅니다.

다음 사람에게: 금액을 시험할 때 **통상임금 바닥 위의 사람도 한 명** 세우십시오. 바닥에 걸리면 평균임금을
어느 창으로 셌는지가 금액에 드러나지 않습니다.

#### 리뷰가 잡은 것 (23) — 세금 경고가 급여 탭과 다른 기본금으로 셌습니다

설정의 세금 경고(`taxWarn`)는 (21)이 '오늘이 맞는 자리'로 센 열네 곳 가운데 하나였지만, 그만둔
사람의 지금 기간이 굳어 있으면 틀렸습니다: 급여 탭은 굳은 기본금으로 세금을 매기는데 경고는 새 회사
기본금으로 "범위를 넘습니다"라고 했습니다. (21)이 예상 실수령에서 고친 바로 그 경우입니다. 이제
`payCalc(totals(), wageOn())`. 스크립트로 다시 세어, **W 없는 호출은 13곳** — 4대보험 설정 전환(2),
설정 탭의 4대보험·보수월액(4)과 지금 회사 휴업수당 안내(4), 진행 중인 근무(2), `t.pay`(1).

#### 리뷰가 잡은 것 (24) — 급여 탭의 '지난 기간 · 앱 계산'이 그 기간의 근무내역서보다 많았습니다

이 줄은 근로자가 지난 명세서와 맞대 보라고 주는 금액입니다. 그 합을 만들던 `prevTotals()`는 잔업·야간·특근
시간만 모으고 결근·휴업 날수와 법정 휴일 8시간 초과를 모으지 않아서, `payCalc`가 결근 공제·휴업 공제·
×2.0 가산을 셀 수 없었습니다. 휴업·결근이 하루씩 있는 8월에 리뷰가 잰 값은 이 줄 ₩3,281,250, 그 달의 근무내역서
₩3,209,440 — 명세서와 맞대면 받지 않아야 할 차액이 보입니다. 이제 그 기간의 `totals()`를 그대로 씁니다.
`prevTotals()`는 이 자리 말고 쓰는 곳이 없어 지웠습니다.
