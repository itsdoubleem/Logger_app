...
₩57,792). 이제 그것도 W로 셉니다. W를 받는 함수(`dayPay`, `legalGap`, `avgDaily`, `avgIsEst`,
`partialShort`, `partialWorkedPay`, `shutdownPay`)를 W 없이 부르는 곳은 이제 셋이고, 셋 다 맞습니다:
설정 탭의 지금 회사 안내(오늘이 맞습니다), `totals()`의 `t.pay`(어디서도 읽지 않는 합 — 돈은
`payCalc(t, W)`가 시간에서 W로 다시 셉니다), 출퇴근 화면의 '마지막 근무'(W가 없으면 `dayPay`는 그
기록에 박힌 그 날의 금액을 씁니다 — 시험이 붙듭니다).
